...
from featuretrack.trackers.algorithms import lucas_kanade, siftbf, kalman
...
def add_siftbf_subparser(subparsers):
  siftbf_parser = subparsers.add_parser(
    'siftbf', help='Brute force tracking based on L2-distance between SIFT point descriptors',
  ...
  siftbf_parser.set_defaults(func=siftbf)
  siftbf_parser.set_defaults(features='sift')
  siftbf_parser.add_argument('-p', '--points', default=100, type=int, help='Number of keypoints to extract [default: 100]')
  siftbf_parser.add_argument('-i', '--interval', default=60, type=int,
                           help='Interval of frames in which feature points are tracked before being recomputed [default: 60]')
  siftbf_parser.add_argument('-t', '--thresh', default=90, type=int,
                           help='Matching quality threshold (distance) [default: 90]')
  siftbf_parser.add_argument('--hidematch', default=False, action='store_true',
                           help='If specified hides reference and current frames with the explicit matching')
  return siftbf_parser
...
def prepare_parser():
  ...
  parser.add_argument('-f', '--framerate', default=30, type=int, help='Output video framerate [default: 30]')
  ...
  add_siftbf_subparser(subparsers)
